data WORK.MDIS;
    ...
    infile 'D:\이경재\학술대회 및 논문공모전\2027 Journal of Regional Science\분석\지역별 고용조사\2011.csv'
        delimiter = ','
        MISSOVER
        DSD
        lrecl=32767
    ...
    label
        ...
        C41 = '27.근로기간 계약여부'
        C42 = '27-1.근로기간'
        C43 = '29.배우자가 수입 목적으로 일한 적 있었는지 여부'
        C44 = '29.1.배우자 총 일한 시간'
        C45 = '사업체소재지코드'
        C46 = '교육정도컨버전'
        C47 = '시도 및 전국 가중치'
        C48 = '경제활동구분'
...
area1 = input(vvalue(c45), best.);

/* ------------------------------------------------------------
   2011년은 c36이 이미 산업대분류 코드임
   - c36: 산업코드, A~U
...
/* ------------------------------------------------------------
   ...
   - wage_wide11  : area1-year별 job 평균임금
   - employ_wide11: area1-year별 job 가중 종사자수
------------------------------------------------------------ */
...
proc summary data=job11_ready nway;
    class area1 year ac job_l;   /* ← ac 추가가 핵심 */
    var wage one;
    weight wt;
    output out=cell_stats11(drop=_TYPE_ _FREQ_)
        mean(wage) = mean_wage
        sum(one)   = employ;
run;

proc sort data=cell_stats11;
    by area1 year ac;            /* ← ac 추가 */
run;

proc transpose data=cell_stats11 out=wage_wide11(drop=_NAME_);
    by area1 year ac;            /* ← ac 추가 */
    id job_l;
    var mean_wage;
run;

proc transpose data=cell_stats11 out=employ_wide11(drop=_NAME_);
    by area1 year ac;            /* ← ac 추가 */
    id job_l;
    var employ;
run;
