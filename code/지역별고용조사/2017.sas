data WORK.MDIS;
    ...
    infile 'D:\이경재\학술대회 및 논문공모전\2027 Journal of Regional Science\분석\지역별 고용조사\2017.csv'
        delimiter = ','
        MISSOVER
        DSD
        lrecl=32767
...
/* ------------------------------------------------------------
   ...
   - wage_wide17  : area1-year별 job 평균임금
   - employ_wide17: area1-year별 job 가중 종사자수
------------------------------------------------------------ */
...
proc summary data=job17_ready nway;
    class area1 year ac job_l;   /* ← ac 추가가 핵심 */
    var wage one;
    weight wt;
    output out=cell_stats17(drop=_TYPE_ _FREQ_)
        mean(wage) = mean_wage
        sum(one)   = employ;
run;

proc sort data=cell_stats17;
    by area1 year ac;            /* ← ac 추가 */
run;

proc transpose data=cell_stats17 out=wage_wide17(drop=_NAME_);
    by area1 year ac;            /* ← ac 추가 */
    id job_l;
    var mean_wage;
run;

proc transpose data=cell_stats17 out=employ_wide17(drop=_NAME_);
    by area1 year ac;            /* ← ac 추가 */
    id job_l;
    var employ;
run;
